...
import { getMyBookingsAPI, cancelBookingAPI, getBookingPaymentUrlAPI } from "@/lib/api/bookings";
...
const BOOKING_STATUS_LABELS: Record<string, { text: string; color: string }> = {
  "PENDING": { text: "Đang xử lý", color: "bg-yellow-100 text-yellow-700" },
...
function ProfileBookingCard({ 
  ...
  const statusBadge = () => {
    if (isPaid)
      return (
        <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full uppercase bg-green-100 text-green-700">
          Đã thanh toán
        </span>
      );
    if (isCancelled || (isPending && isExpired))
      return (
        <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full uppercase bg-red-100 text-red-700">
          Đã hủy
        </span>
      );
    if (isPending)
      return (
        <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full uppercase bg-yellow-100 text-yellow-700">
          Đang xử lý
        </span>
      );
    return (
      <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full uppercase bg-gray-100 text-gray-700">
        {booking.status}
      </span>
    );
  };
  ...
  return (
    <div className="border border-gray-100 rounded-xl p-5 shadow-sm hover:shadow-md transition flex flex-col bg-white gap-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex-grow">
          ...
          <div className="text-gray-500 text-sm mt-1 flex flex-wrap gap-x-6 gap-y-1">
            <span>
              📅 Ngày đặt:{" "}
              {booking.bookingDate
            ...
            <span>
              👥 Số lượng: {booking.quantity} ({booking.adults} NL,{" "}
              {booking.children} TE)
            </span>
            <span>
        ...
        <div className="flex flex-row md:flex-col justify-between items-end gap-2 text-right shrink-0">
          <div>
          ...
          {isPending && !isExpired && (
            <button
              ...
              className="mt-2 px-4 py-1.5 text-xs font-bold text-red-600 bg-red-50 border border-red-100 hover:bg-red-100 rounded-lg transition"
            >
              ✕ Hủy Tour
            </button>
          )}
      ...
      {isPending && !isExpired && (
        <div className="p-4 bg-amber-50 rounded-xl border border-amber-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex-grow">
            <p className="text-xs text-amber-600 font-semibold mb-1">
              ⏰ Thời gian giữ chỗ còn lại: <span className="font-mono font-bold">{formattedTime}</span>
            </p>
          ...
          <div className="shrink-0">
            {currentPaymentUrl ? (
              <button
                ...
                className="px-5 py-2 bg-gradient-to-r from-sky-500 to-indigo-500 hover:from-sky-600 hover:to-indigo-600 text-white font-bold text-sm rounded-xl shadow hover:shadow-md transition-all hover:scale-[1.02] whitespace-nowrap"
              >
              ...
              <button
                disabled
                className="px-5 py-2 bg-gray-200 text-gray-500 font-bold text-sm rounded-xl cursor-not-allowed flex items-center gap-2 whitespace-nowrap"
              >
