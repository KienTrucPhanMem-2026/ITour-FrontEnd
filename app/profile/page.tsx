              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                {bookings.length === 0 ? (
                  ...
                  <div className="space-y-5">
                    {bookings.map(b => {
                      const statusInfo = BOOKING_STATUS_LABELS[b.status] || { text: b.status, color: "bg-gray-100 text-gray-700" };
                      
                      return (
                        <div key={b.bookingId} className="border border-gray-100 rounded-xl p-5 shadow-sm hover:shadow-md transition flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white">
                          <div className="flex-grow">
                            <div className="flex items-center gap-3 mb-2">
                              <span className="text-[10px] font-black bg-gray-900 text-white px-2 py-0.5 rounded uppercase">{b.bookingId}</span>
                              <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full uppercase ${statusInfo.color}`}>
                                {statusInfo.text}
                              </span>
                            </div>
                            <h3 className="font-bold text-gray-900 text-lg">{b.tourName || "Tên Tour"}</h3>
                            <div className="text-gray-500 text-sm mt-1 flex flex-wrap gap-x-6 gap-y-1">
                              <span>📅 Ngày đặt: {b.bookingDate ? new Date(b.bookingDate).toLocaleDateString("vi-VN") : "N/A"}</span>
                              <span>👥 Số lượng: {b.quantity} ({b.adults} NL, {b.children} TE)</span>
                              <span>💳 {PAYMENT_METHOD_LABELS[b.paymentMethod] || b.paymentMethod}</span>
                            </div>
                          </div>
                          
                          <div className="flex flex-row md:flex-col justify-between items-end gap-2 text-right shrink-0">
                            <div>
                              <div className="text-gray-400 text-[10px] font-bold uppercase">Tổng tiền</div>
                              <div className="text-xl font-black text-gray-900">
                                {b.finalPrice ? (b.finalPrice).toLocaleString("vi-VN") + "₫" : "N/A"}
                              </div>
                            </div>
